...
const modifierBulletin = (req, res) => {
    const { id: id } = req.params;
    const { filiere, nom_module, code_module, coefficient, note } = req.body;
    const sql = 'UPDATE notes SET filiere = ?, nom_module= ?, code_module= ?, coefficient = ?, note = ? WHERE num_etudiant = ?';
    const values = [filiere, nom_module, code_module, coefficient, note, id];
    pool.getConnection((err, connection) => {
        if (err) throw err;
        connection.query(sql, values, (err, data) => {
            if (err)
...
const supprimerBulletin = (req, res) => {
    ...
    pool.getConnection((err, connection) => {
        if (err) throw err;
        connection.query('DELETE FROM notes WHERE num_etudiant = ?', [id], (err, data) => {
            if (err)
                return res.status(500).json({ success: false });
